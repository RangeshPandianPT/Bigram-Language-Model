...
- ✅ Trained a dedicated draft model via `scripts/train_draft.py` to make speculative decoding effective.
...
## 🚀 Level 7: The Frontier (Proposed Next Steps)

Now that the core LLM, fine-tuning pipeline, and deployment are complete, the project can expand into cutting-edge AI capabilities. Here are several exciting paths we can take next:

### Path A: Multimodal Vision-Language Model (VLM) ✅
- **Goal:** Give the model "eyes" by integrating an image encoder.
- **Tasks:** ✅ Implemented `llm.vlm.VLM` bridging a dummy vision encoder to the LLM's embedding space in `src/llm/vlm.py`.
...
- **Tasks:** ✅ Implemented `llm.mamba.MambaLanguageModel` demonstrating an SSM block with linear time complexity scaling in `src/llm/mamba.py`.
